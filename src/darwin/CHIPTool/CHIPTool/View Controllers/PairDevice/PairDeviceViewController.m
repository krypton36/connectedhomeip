...
@property (nonatomic, strong) UITextField * setupCode;
...
- (void)setupUIElements
{
    ...
    // Title
    _titleLabel = [CHIPUIViewUtils addTitle:@"Pair Device" toView:self.view];
    [self setupStackView];
}
...
- (void)setupStackView
{
    ...
    _setupCode = [UITextField new];

    UILabel * setupCodeLabel = [UILabel new];
    setupCodeLabel.text = @"Setup Code:";
    UIView * setupCodeView = [CHIPUIViewUtils viewWithLabel:setupCodeLabel textField:_setupCode];
    [stackView addArrangedSubview:setupCodeView];

    setupCodeView.translatesAutoresizingMaskIntoConstraints = false;
    [setupCodeView.trailingAnchor constraintEqualToAnchor:stackView.trailingAnchor].active = true;

    // Unpair devices button
    _unpairButton = [UIButton new];
    [_unpairButton setTitle:@"Pair Device" forState:UIControlStateNormal];
    [_unpairButton addTarget:self action:@selector(pairDevice:) forControlEvents:UIControlEventTouchUpInside];
...
- (IBAction)pairDeviceButton:(id)sender
{
