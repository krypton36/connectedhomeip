...
{
    EmberAfStatus status = Clusters::DoorLock::Attributes::LockType::Set(kLockEndpointId, DlLockType::kDeadBolt);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        LOG_ERR("Updating type %x", status);
    }
    // write the new LockState values
    
    status = Clusters::DoorLock::Attributes::LockState::Set(kLockEndpointId, BoltLockMgr().IsUnlocked() ? DlLockState::kLocked:DlLockState::kUnlocked);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        LOG_ERR("Updating state %x", status);
    }
}
